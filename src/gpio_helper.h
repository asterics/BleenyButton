...
int dRead( uint32_t ulPin )
{
  if (ulPin >= NUMBER_OF_PINS) return 0;
...
void dWrite( uint32_t ulPin, uint32_t ulVal )
{
  if (ulPin >= NUMBER_OF_PINS) return;

  NRF_GPIO_Type * port = nrf_gpio_pin_port_decode(&ulPin);

  if(ulVal) port->OUTSET = (1UL << ulPin);
  else port->OUTCLR = (1UL << ulPin);
}


void dMode( uint32_t ulPin, uint32_t ulMode )
{
  if (ulPin >= NUMBER_OF_PINS) return;
...
void dToggle( uint32_t pin )
{
  int state = 1 - dRead(pin);
  dWrite(pin, state);
}
